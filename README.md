...
| 항목 | 내용 |
|------|------|
| ⭐ **비회원 예매** | `users.is_guest` 플래그로 단일 테이블 설계, 이름+연락처로 예매 조회 |
| ⭐ **41개 테이블 4단계 분리** | MVP → 운영 → 마케팅 → 편의 순서로 점진 확장 |
| ⭐ **코드 테이블 패턴** | 14개 enum → Lookup Table 전환, 운영 중 값 추가/수정 가능 |
| ⭐ **좌석 동시성 처리** | SeatHold + UNIQUE 제약 + 10분 TTL, 이중 결제 원천 차단 |
| ⭐ **이력 관리** | 포인트/쿠폰/영수증/티켓/좌석변경 모두 immutable 이력 테이블 |
| ⭐ **비정규화 캐싱** | avg_rating, review_count, booking_rank 등 집계 캐시 컬럼 |
