...
import { getTheaters } from '../api/movies'
...
export default function TheaterList() {
  ...
  useEffect(() => {
    getTheaters()
      .then((res) => {
        ...
        const firstWithTheater = REGION_ORDER.find((r) => res.data.some((t) => t.region === r))
        if (firstWithTheater) setSelectedRegion(firstWithTheater)
      })
      .finally(() => setLoading(false))
  }, [])

  const regionCount = theaters.reduce<Record<string, number>>((acc, t) => {
    acc[t.region] = (acc[t.region] ?? 0) + 1
    return acc
  }, {})

  const theatersInRegion = theaters.filter((t) => t.region === selectedRegion)
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      <h1 className="text-xl font-bold text-gray-900 mb-5">극장 선택</h1>
        ...
        <div className="flex border border-gray-200 rounded-xl overflow-hidden bg-white" style={{ minHeight: '24rem' }}>
          ...
          <div className="flex-1 p-4">
            {theatersInRegion.length === 0 ? (
              <p className="text-sm text-gray-400 pt-6">해당 지역에 극장이 없습니다.</p>
            ) : (
