...
export default function Signup() {
  ...
  const inputClass = 'w-full border border-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500'
  const errorClass = 'text-red-500 text-xs mt-1'

  return (
    <div className="min-h-screen flex items-center justify-center bg-mist py-12">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-ink-soft mb-1">비밀번호 확인 *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-ink-soft mb-1">전화번호</label>
            <input {...register('phone')} type="tel" placeholder="010-0000-0000" className={inputClass} />
          </div>

          {/* 약관 동의 */}
          <div className="border border-line rounded-xl p-4 space-y-1">
            {/* 전체 동의 */}
            <label className="flex items-center gap-2 text-sm font-semibold text-ink pb-2 mb-1 border-b border-line cursor-pointer">
              <input
                type="checkbox"
                ...
                <div className="flex items-center justify-between py-1.5">
                  <label className="flex items-center gap-2 text-sm cursor-pointer flex-1 min-w-0">
                    <input
                      {...register(term.id)}
                      type="checkbox"
                      className="rounded accent-gold-500 flex-shrink-0"
                    />
                    <span className="truncate">
                      <span className={`mr-1 ${term.required ? 'text-red-500' : 'text-ink-faint'}`}>
                        [{term.required ? '필수' : '선택'}]
                      </span>
                      {term.label}
                    </span>
                  ...
                  <button
                    ...
                    className="text-xs text-ink-faint hover:text-ink-soft underline ml-2 flex-shrink-0"
                  >
                    보기
                  </button>
                </div>
        ...
        <p className="text-center text-sm text-ink-faint mt-6">
          이미 계정이 있으신가요?{' '}
          <Link href="/login" className="text-gold-700 hover:underline font-medium">로그인</Link>
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-line flex-shrink-0">
              <h3 className="text-sm font-bold text-ink">{viewingTerm.label}</h3>
              <button onClick={() => setViewingTerm(null)} className="text-ink-faint hover:text-ink-soft text-xl leading-none">×</button>
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-5 text-sm text-ink-soft whitespace-pre-line leading-relaxed">
              {viewingTerm.content}
            </div>
            <div className="px-5 pb-5 flex-shrink-0">
              <button
                onClick={() => setViewingTerm(null)}
                className="w-full bg-gilt hover:brightness-105 text-white py-2.5 rounded-xl text-sm font-medium"
              >
                확인
              </button>
            </div>
